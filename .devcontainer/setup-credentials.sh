...
setup_mcp_servers() {
    ...
    if [ -n "${ATLASSIAN_USER_EMAIL:-}" ] && [ -n "${ATLASSIAN_API_TOKEN:-}" ] && [ -n "${ATLASSIAN_SITE_NAME:-}" ]; then
        ...
        claude mcp add-json confluence "{\"type\":\"stdio\",\"command\":\"mcp-atlassian-confluence\",\"env\":${ATLASSIAN_ENV}}" --scope user >/dev/null
        claude mcp add-json jira "{\"type\":\"stdio\",\"command\":\"mcp-atlassian-jira\",\"env\":${ATLASSIAN_ENV}}" --scope user >/dev/null
        MCP_SERVERS="$MCP_SERVERS confluence jira"
    fi

    if [ -n "${ATLASSIAN_USER_EMAIL:-}" ] && [ -n "${BITBUCKET_API_TOKEN:-}" ]; then
        claude mcp add-json bitbucket '{"type":"stdio","command":"mcp-atlassian-bitbucket","env":{"ATLASSIAN_USER_EMAIL":"${ATLASSIAN_USER_EMAIL}","ATLASSIAN_API_TOKEN":"${BITBUCKET_API_TOKEN}"}}' --scope user >/dev/null
        MCP_SERVERS="$MCP_SERVERS bitbucket"
    fi

    if [ -n "${GITHUB_PERSONAL_ACCESS_TOKEN:-}" ]; then
        claude mcp add-json github '{"type":"stdio","command":"mcp-server-github","env":{"GITHUB_PERSONAL_ACCESS_TOKEN":"${GITHUB_PERSONAL_ACCESS_TOKEN}"}}' --scope user >/dev/null
        MCP_SERVERS="$MCP_SERVERS github"
    fi

    # Context7: live library docs (no credentials required)
    claude mcp add-json context7 '{"type":"http","url":"https://mcp.context7.com/mcp"}' --scope user >/dev/null
