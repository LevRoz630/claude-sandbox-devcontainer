...
MCP servers (Confluence, Jira, Bitbucket, GitHub) are configured automatically from env vars — whether those come from `.env`, host environment, or 1Password. `setup-env.sh` registers them via `claude mcp add-json --scope user` with `${VAR}` references (no plaintext secrets on disk).
...
- Registers MCP servers via `claude mcp add-json` with `${VAR}` env references (no plaintext secrets on disk)
